...
semantics(porset_copies, storage) -> ram_copies;
semantics(porset_copies, types  ) -> [set, ordered_set, bag];
semantics(porset_copies, index_types) -> [ordered];
semantics(_Alias, _) ->
    undefined.
...
load_table(porset_copies, _Tab, init_index, _Cs) -> ok;
...
insert(Alias, Tab, Obj) ->
    io:format("running my own lookup function~p~n", [Alias]),
    ?DBG({Tab,Obj}),
    try
...
select({porset_copies, C}) ->  ets:select(C).
